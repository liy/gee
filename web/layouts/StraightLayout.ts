...
class LaneAllocator {
  ...
  activeBranches = Array<boolean>();

  constructor() {
    this.activeBranches = new Array<boolean>();
  }
    ...
    for (let i = 0; i < priorityBranchSections.length; ++i) {
      ...
      if (branchTip.y < start) {
        ...
        while (!visited.has(branchBase) && branchBase) {
          branchBase = branchBase.parentNodes[0];
  ...
  process(): LayoutResult {
    ...
    for (let i = 0; i < this.priorityBranches.length; ++i) {
      ...
      if (!visited.has(node)) {
        ...
        while (true) {
          ...
          if (node.parentNodes[0] === undefined) break;
    ...
    for (let y = 0; y < this.graph.size; ++y) {
      ...
      for (let i = 1; i < node.parentNodes.length; ++i) {
        let branchBase = node.parentNodes[i];
